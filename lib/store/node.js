...
class NodeIndexer {
  ...
  async indexLastStatus(batch, entry) {
    ...
    batch.put(nodes.STATUS_10_BY_TIME.encode(min10, entry.key), encoded);
  }
  ...
  async indexHourly(batch, entry) {
    ...
    batch.put(nodes.STATUS_HOUR_BY_TIME.encode(min60, entry.key), encoded60);
  ...
  async indexDaily(batch, entry) {
    ...
    batch.put(nodes.STATUS_DAY_BY_TIME.encode(minDay, entry.key), encodedDay);
