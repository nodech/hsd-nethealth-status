...
class DNSIndexer {
  ...
  async indexLastStatus(batch, entry) {
    ...
    batch.put(dns.STATUS_10_BY_TIME.encode(min10, entry.key), entry.encode());
  }
  ...
  async indexHourly(batch, entry) {
    ...
    batch.put(dns.STATUS_HOUR_BY_TIME.encode(min60, entry.key), encoded60);
  ...
  async indexDaily(batch, entry) {
    ...
    batch.put(dns.STATUS_DAY_BY_TIME.encode(bucketDay, entry.key), encodedDay);
